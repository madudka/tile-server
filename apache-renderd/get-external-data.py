...
class Downloader:
    ...
    def download(self, url, name, opts, data_dir, table_last_modified):
        ...
        #Полностью пропустить, если файл уже есть
        if opts.skip_if_cached and os.path.exists(filename):
            logging.info("  Skipping {} (already cached, --skip-if-cached)".format(name))
            try:
                ...
                lastmod = None
            with open(filename, 'rb') as fp:
                return DownloadResult(status_code=200, content=fp.read(), last_modified=lastmod)

        # Использовать кэш без HTTP-запроса
        if opts.skip_http_check and os.path.exists(filename):
            logging.info("  Using cached file for {} (--skip-http-check)".format(name))
            try:
                with open(filename_lastmod, 'r') as fp:
                    lastmod = fp.read()
            except FileNotFoundError:
        ...
        # Стандартная логика кэша
        if os.path.exists(filename) and os.path.exists(filename_lastmod):
            with open(filename_lastmod, 'r') as fp:
                lastmod_cache = fp.read()
            with open(filename, 'rb') as fp:
                cached_data = DownloadResult(status_code=200, content=fp.read(), last_modified=lastmod_cache)
        else:
            cached_data = None
            lastmod_cache = None
        ...
        # Если отключена HTTP-проверка и есть кэш — используем его
        if opts.skip_http_check and cached_data is not None:
            logging.info("  Using cached file for {} (HTTP check skipped)".format(name))
            return cached_data
        ...
        if opts.no_update and (cached_data or table_last_modified):
            ...
            # Только при отключённой опции skip_http_check делаем HTTP-запрос
            if opts.skip_http_check:
                # Если кэша нет — всё равно скачиваем, но без заголовков
                headers = {}
...
def main():
    ...
    parser.add_argument("--skip-http-check", action="store_true",
                    help="Skip HTTP modification check entirely; use only local cache if available")
    parser.add_argument("--skip-if-cached", action="store_true",
                    help="Skip download and import if the file is already present in the cache")
